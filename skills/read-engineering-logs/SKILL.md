...
Search and retrieve context from past Obsidian scratchpad entries **and prerendered agent sessions** using `qmd`. **Default to `~/.claude/skills/read-engineering-logs/search.sh "<query>"`** — it handles freshness (update + embed) and runs full semantic search (1.7B expansion + reranking). Pass `--lean` for M1 Pro or constrained machines (0.6B Qwen3, no reranking). This skill is read-only — it searches and synthesizes past entries but does not write new ones. For writing, see `write-engineering-logs`.

The search covers two corpora:
- **scratchpads** — curated engineering logs agents write intentionally
- **sessions** — prerendered agent session transcripts (Claude, Pi, Codex) generated by `read-agent-sessions prerender`
