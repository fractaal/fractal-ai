...
if (Test-Path -Path $agentsSource -PathType Leaf) {
    ...
    Link-FractalItem -Source $agentsSource -Target (Join-Path (Join-Path $HOME '.gemini') 'AGENTS.md')
    Link-FractalItem -Source $agentsSource -Target (Join-Path (Join-Path $HOME '.gemini') 'CLAUDE.md')
}
