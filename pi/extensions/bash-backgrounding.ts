...
const bashParams = Type.Object({
	...
	background_after_seconds: Type.Optional(Type.Number({
...
const bashOutputParams = Type.Object({
	...
	wait_seconds: Type.Optional(Type.Number({
		description: "When block=true, seconds to wait for the background task to finish before returning current status and output. This only controls this polling call. It does not kill the task. Default: 60. Maximum: 60.",
	})),
...
function scheduleUpdate(onUpdate: ((result: any) => void) | undefined, task: TaskRecord): void {
	if (!onUpdate) return;
	const emit = () => {
		lastUpdateAt = Date.now();
		onUpdate({ content: [{ type: "text", text: snapshotTail(task) || "(running...)" }], details: { outputPath: task.outputPath } });
	};
...
function finishTask(pi: ExtensionAPI, task: TaskRecord, status: Exclude<TaskStatus, "running">, exitCode: number | null): void {
	if (task.completionSettled) return;
...
function containsUnsupportedDetach(command: string): boolean {
	return /(^|[;&|\s])(?:\S*\/)?(nohup|disown|setsid|systemd-run)(\s|$)/.test(command)
...
function rejectLegacyTimeout(params: Record<string, unknown>, toolName: "bash" | "bash_output"): void {
	...
	if (toolName === "bash") {
		throw new Error("bash.timeout has been removed because it was ambiguous. Use background_after_seconds to control foreground responsiveness, or kill_after_seconds for an explicit hard kill deadline.");
	}
	throw new Error("bash_output.timeout has been removed because it was ambiguous. Use wait_seconds to control this polling wait; use kill_bash to stop the task.");
}
...
function resolveBashOutputWaitSeconds(value: number | undefined): number {
	const waitSeconds = parsePositiveSeconds(value, "wait_seconds", MAX_FOREGROUND_WAIT_SECONDS)!;
...
export default function bashBackgrounding(pi: ExtensionAPI) {
	pi.registerTool({
		...
		async execute(_toolCallId, params, signal, onUpdate, ctx) {
			...
			if (containsUnsupportedDetach(params.command)) {
				throw new Error("Unsupported shell detach pattern. Do not use nohup, disown, setsid, systemd-run, docker -d/--detach, or daemonizing commands with Pi bash; run the command normally and let bash return a tracked task_id.");
			}
			const { backgroundAfterSeconds, killAfterSeconds } = resolveBashTiming(params);
			const task = await spawnTask(pi, params.command, ctx.cwd, backgroundAfterSeconds, killAfterSeconds, params.description);
			const abort = () => {
				requestKill(pi, task, "aborted by user");
			};
			if (params.run_in_background === true) {
				task.notifyOnCompletion = true;
				return { content: [{ type: "text", text: backgroundText(task, false) }], details: { outputPath: task.outputPath } };
			}
			...
			try {
				...
				if (completion === null) {
					task.notifyOnCompletion = true;
					return { content: [{ type: "text", text: backgroundText(task, true) }], details: { outputPath: task.outputPath } };
				}
	...
	pi.registerTool({
		name: "bash_output",
		label: "bash_output",
		description: "Read output and status for a background bash task. Set block=true to wait briefly for completion; wait_seconds controls only this polling wait and does not kill the task.",
		...
		async execute(_toolCallId, params) {
			...
			if (params.block === true && task.status === "running") {
	...
	pi.on("session_shutdown", async () => {
		...
		await Promise.race([
			Promise.allSettled([...tasks.values()].map((task) => task.completion)),
			delay(2_000),
		]);
		for (const task of tasks.values()) {
			if (task.status !== "running") {
