...
async function generateFractalSummary(pi: ExtensionAPI, ctx: ExtensionContext, event: Parameters<Parameters<ExtensionAPI["on"]>[1]>[0] & any): Promise<string> {
	...
	const maxTokens = Math.min(Math.floor(0.9 * settings.reserveTokens), ctx.model.maxTokens > 0 ? ctx.model.maxTokens : Number.POSITIVE_INFINITY);
...
export default function fractalCompactExtension(pi: ExtensionAPI) {
	pi.on("session_before_compact", async (event, ctx) => {
		...
		try {
			...
			if (!event.signal.aborted) ctx.ui.notify(`Fractal compaction failed; compaction cancelled instead of using Pi default: ${message}`, "error");
			return { cancel: true };
		} finally {
